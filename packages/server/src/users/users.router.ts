...
export const usersRouter = router({
        ...
        const user = await usersRepository.create({
          ...
          notesLimit:
            input.subscription === 'free'
              ? 1000
              : input.subscription === 'pro'
                ? 10000
                : input.subscription === 'team'
                  ? 50000
                  : 1000000,
          storageLimit:
            input.subscription === 'free'
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (updateData.username) {
          const existingUser = await usersRepository.findByUsername(
            updateData.username,
          );
  ...
  updateSettings: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const user = await usersRepository.updateSettings(
          input.userId,
          input.settings,
        );
  ...
  updatePreferences: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const user = await usersRepository.updatePreferences(
          input.userId,
          input.preferences,
        );
  ...
  updateStorageUsage: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        await usersRepository.updateStorageUsage(
          input.userId,
          input.sizeChange,
        );
        return { success: true };
  ...
  getInvites: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        const invites = await usersRepository.getInvites(
          input.userId,
          input.status,
        );
        return invites;
  ...
  updateInviteStatus: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const invite = await usersRepository.updateInviteStatus(
          input.inviteId,
          input.status,
        );
  ...
  // Admin Operations
  list: protectedProcedure.input(ListUsersSchema).query(async ({ input }) => {
    try {
