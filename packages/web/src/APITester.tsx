...
export function APITester() {
  ...
  async function fetchPing() {
    try {
      const response = await trpc.ping.query(); // 请求 /trpc/ping
      console.log('Response from /trpc/ping:', response);
      return response;
    } catch (error) {
      console.error('Error fetching /trpc/ping:', error);
    }
  }
  ...
  const testEndpoint = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (method === "tRPC") {
        const pingResponse = await fetchPing();
    ...
    <div className="mt-8 mx-auto w-full max-w-2xl text-left flex flex-col gap-4">
      ...
      >
        ...
        >
          ...
          <option value="tRPC" className="py-1">
            tRPC
          </option>
        </select>
        <input
