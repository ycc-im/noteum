...
export interface SyncStrategy {
  ...
  ): Promise<{ resolvedValue: any; strategy: ConflictResolutionStrategy }>;
  ...
  private async applyDefaultResolution(
    ...
    switch (strategy) {
      ...
      case 'merge':
        ...
        return this.applyDefaultResolution({
          ...conflict,
          defaultStrategy: 'timestamp',
        } as any);
...
export class BatchedSyncStrategy implements SyncStrategy {
  ...
  async queueOperation(operation: SyncOperation): Promise<void> {
    ...
    if (
      operation.priority >= this.batchConfig.immediateThreshold ||
  ...
  async resolveConflict(
    ...
    const immediateStrategy = new ImmediateSyncStrategy(
      () => Promise.resolve(),
      this.conflictConfig
    );
...
export class IntelligentSyncStrategy implements SyncStrategy {
  ...
  constructor(
    private syncCallback: (
      operation: SyncOperation | SyncOperation[]
    ) => Promise<void>,
    private batchConfig: BatchSyncConfig,
    private conflictConfig: ConflictResolutionConfig
  ) {
  ...
  private adaptStrategy(): void {
    ...
    this.adaptationMetrics.conflictRate =
      immediateMetrics.conflictsDetected /
      Math.max(immediateMetrics.totalOperations, 1);
    ...
    const targetStrategy = shouldUseBatched
      ? this.batchedStrategy
      : this.immediateStrategy;

    if (this.currentStrategy !== targetStrategy) {
      console.debug(
        `[IntelligentSync] Switching to ${targetStrategy.type} strategy`
      );
  ...
  objectMergeResolver: async (conflict: ConflictInfo) => {
    if (
      ...
      return {
        ...
        strategy: 'merge' as ConflictResolutionStrategy,
      };
    }
