...
export const ulidSchema = z
  .string()
  .refine(value => UlidGenerator.isValid(value), {
    message:
 ...
 */
export const optionalUlidSchema = z
  .string()
  .refine(
    value =>
      value === undefined || value === null || UlidGenerator.isValid(value),
    {
      message:
        "Invalid ULID format. Expected 26-character string using Crockford's base32 alphabet.",
    }
 ...
 */
export const uuidToUlidSchema = z
  .string()
  .refine(
    value => {
...
export const createUlidSchema = (fieldName?: string) => {
  ...
  return z.string().refine(value => UlidGenerator.isValid(value), { message });
};
